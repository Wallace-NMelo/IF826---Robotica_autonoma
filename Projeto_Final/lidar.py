...
def merge(x, y):
    ...
    for i in range(len(output_list_x)):
        #plt.scatter(output_list_x[i], output_list_y[i])
        #plt.show()
        ...
        output_lines = np.hstack((output_lines, np.array([ang_coef, lin_coef]))) if output_lines.size else np.array([ang_coef, lin_coef])
    #print((R[0])[1][1])    
    return output_lines
...
# def is_matched(vji,cov, vij):
    # g = 0.5
    # return vji*cov*vij < g^2
    
# def global_to_robot(alpha, World_r, x_robot, y_robot, theta_robot):
    # return np.array([alpha - theta, World_r - (x*np.cos(alpha) + y*sin(alpha))]);
